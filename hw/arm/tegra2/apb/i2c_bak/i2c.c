                            ...
                            uint64_t value, unsigned size)
{
    ...
    case 0x00 /* I2C_CNFG */:
        ...
        if (s->config & (1<<9)) { // Normal mode
            ...
            printf("tegra_i2c_write: addr=0x%x, is_read=%d\n", addr, is_read);
            if (s->config & (1<<4)) {
                printf("tegra_i2c_write: I2C_CNFG SLV2=1, slave2 addr=0x%x\n", s->cmd_addr1);
            }
