...
typedef struct tegra_ictlr_state {
    ...
    MemoryRegion iomem;
    uint32_t virq_cpu[MAX_BANKS];
    uint32_t virq_cop[MAX_BANKS];
    uint32_t vfiq_cpu[MAX_BANKS];
    uint32_t vfiq_cop[MAX_BANKS];
    uint32_t isr[MAX_BANKS];
    uint32_t fir[MAX_BANKS];
    uint32_t cpu_ier[MAX_BANKS];
    uint32_t cpu_iep_class[MAX_BANKS];
    uint32_t cop_ier[MAX_BANKS];
    uint32_t cop_iep_class[MAX_BANKS];
} tegra_ictlr;
